...
(function (root, factory) {
  ...
  var WORK_LINES = {
    ...
    'sales-hilura': [
      '依頼を案件依頼か制作関連かに分類中',
      '納期の可否は数字で出す。印象では言わない',
      'カレンダーが読めないときは算出不可と書く'
    ],
    'sales-chat': [
      '指示を確定・要確認・解釈が割れる点に切り分け中',
      '原文は書き換えない。要約は別に作る',
      '曖昧な箇所は埋めずに確認へ立てる'
    ]
  };
  ...
  var SCENES = [
    ...
    {
      id: 'intake-classify',
      from: 'sales-hilura', to: 'director',
      source: 'ヒルウラ窓口の役割定義',
      lines: [
        ['sales-hilura', 'ヒルウラから連絡です。修正指示なので制作関連に分類しました'],
        ['director', '受け取る。新規案件として滞留させなかったのが正しい'],
        ['sales-hilura', '納期に関わる部分は数字を添えてあります']
      ]
    },
    {
      id: 'intake-schedule',
      from: 'sales-hilura', to: 'cto', place: 'meeting',
      source: '納期分析の完了条件',
      lines: [
        ['sales-hilura', '案件依頼です。カレンダー未接続なので空き時間を算出できていません'],
        ['cto', '推測で埋めないこと。「確認が必要」のまま上げるのが正しい'],
        ['sales-hilura', '判定は保留にして、不足している情報だけ列挙します']
      ]
    },
    {
      id: 'chat-split',
      from: 'sales-chat', to: 'common-manual',
      source: 'チャット窓口の切り分け',
      lines: [
        ['sales-chat', '指示のこの箇所、2通りに読めます。両方書いて渡します'],
        ['common-manual', '片方に寄せずに来たのは助かります。根拠を引いて判定します'],
        ['sales-chat', '原文もそのまま添えてあります']
      ]
    },
    {
      id: 'auto-roadmap',
